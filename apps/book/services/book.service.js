...
const KEY = 'booksDB'
const GOOGLE_KEY = 'googleBooksDB'
const gBooks = [
  {
    "id": "OXeMG8wNskc",
  ...
  return axios.get(url)
    ...
    .then(books => {
      const newBooks = books.map(book => ({
        ...
        listPrice: {
          amount: utilService.getRandomIntInclusive(20, 100),
          currencyCode: 'ILS',
          isOnSale: utilService.getRandomIntInclusive(0, 1)
        }
      }))
      googleBooks[search] = newBooks
      _saveGoogleBooksToStorage(googleBooks)
...
function getNextBookId(bookId) {
  let books = _loadFromStorage()
  const bookIdx = books.findIndex(book => book.id === bookId)
  let nextBookIdx = (bookIdx === books.length -1) ? 0 : bookIdx + 1
...
function _loadGoogleBooksFromStorage() {
  return storageService.loadFromStorage(GOOGLE_KEY)
}

function _saveGoogleBooksToStorage(books) {
  storageService.saveToStorage(GOOGLE_KEY, books)
}